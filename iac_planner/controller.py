...
class Controller:
    ...
    def run_controller_timestep(self, waypoints=0):
        if waypoints == 0:
            ...
            i = 1
        self.simWait.read()
        for simWaitSample in self.simWait.samples.valid_data_iter:
            print("simWait data received")
            # This where your code goes #
            # vehicleState = data.vehicle_state

            # self.simWait.read()
            self.vehicle_State.wait()
            self.vehicle_State.take()
            for vehicleState in self.vehicle_State.samples.valid_data_iter:
                self.x = vehicleState["cdgPos_x"]
                self.y = vehicleState["cdgPos_y"]
                self.v = vehicleState["cdgSpeed_x"]
                self.yaw = vehicleState["cdgSpeed_heading"]
                gear = vehicleState["GearEngaged"]
                ...
                self.vehicleSteer.instance.setNumber("AdditiveSteeringWheelAngle", self.steer_output)
                print(self.steer_output)
                self.vehicleCorrect.instance.setNumber("AcceleratorAdditive", self.throttle_output)
                # self.vehicleCorrect.instance.setNumber("BrakeAdditive", self.brake_output )
                self.vehicleCorrect.write()
                self.vehicleSteer.write()
